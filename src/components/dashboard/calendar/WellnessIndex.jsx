"use client";
// Top-of-page client snapshot card.
//   1. Number → how is the client
//   2. Arrow  → trending which way
//   3. Badge  → still engaged?
// Click "Show detail" to see latest log vs typical baseline.
import { useState } from "react";
import { A, AD, BG, BO, MU, SU, TX } from "./theme";
import { COMPONENT_KEYS } from "./constants";
import { calculateWellness } from "./wellnessScore";

function tierFor(score, t) {
  if (score >= 80)
    return { color: "#16A34A", label: t.tierPeakForm ?? "Peak form" };
  if (score >= 60)
    return { color: "#7AABDB", label: t.tierOnTrack ?? "On track" };
  if (score >= 40)
    return { color: "#FBBF24", label: t.tierPlateauing ?? "Plateauing" };
  if (score >= 20)
    return { color: "#FB923C", label: t.tierStruggling ?? "Struggling" };
  return { color: "#EF4444", label: t.tierBurnoutRisk ?? "Burnout risk" };
}
...
export default function WellnessIndex({ data, t, month }) {
  ...
  const componentLabels = {
    consistency: t.compConsistency ?? "Consistency",
    soreness: t.compLowSoreness ?? "Low soreness",
    moodEnergy: t.compMoodEnergy ?? "Mood/Energy",
    recovery: t.compRecovery ?? "Recovery",
