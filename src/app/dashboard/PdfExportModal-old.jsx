...
// ── PDF generator ──────────────────────────────────────────────────────────
async function generatePDF({ data, t, year, month, allTime, logs }) {
  await loadScript("https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js");
  await loadScript("https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js");
  await new Promise(r => setTimeout(r, 800));

  const [trendsImg, radarImg, categoryImg, volumeImg, scoresImg, restqImg, goalsImg] =
    await Promise.all([
  ...
  const NAVY = [45, 74, 110], GRAY = [122, 154, 184], LGRAY = [245, 247, 250];
  const DARK = [26, 44, 61], WHITE = [255, 255, 255];
  ...
  const periodLabel = allTime ? (t.allTime ?? "All time") : `${months[month]} ${year}`;
  ...
  // Goal type and Pro status hidden for now — both have a single
  // canonical value across all clients, so they add no signal.
  const patRows = [
  ...
  // Left column — stat rows (manually drawn so we can control the width)
  periodRows.forEach(([label, value], i) => {
    ...
    doc.setFontSize(9); doc.setFont("helvetica", "normal");
    doc.setTextColor(...GRAY); doc.text(String(label), ML + 2, ry2 + 4);
    doc.setTextColor(...DARK); doc.setFont("helvetica", "bold");
    doc.text(String(value ?? "—"), ML + leftColW - 16, ry2 + 4);
  });

  // Right column — goals radar, vertically centered against the rows
  if (hasGoalsRadar) {
  ...
  // ── Full log records ──────────────────────────────────────────────────
  // Always start the log table on a new page — it's a long structured
  // section that reads better as a unit and shouldn't get split awkwardly
  // at the end of a previous page.
  doc.addPage();
  y = 16;
  ...
  if (sorted.length === 0) {
    ...
    sorted.forEach((r, i) => {
      ...
      doc.text(r.isRestDay ? (t.restDay ?? "Rest") : (wText || "—"), cols[7].x, y + 4);
...
function OffscreenCharts({ data, logs, t }) {
  ...
  // Wellbeing radar (5 axes)
  const avg = (key) => {
    const v = logs.map(l => l[key]).filter(x => typeof x === "number");
    return v.length ? v.reduce((a, b) => a + b, 0) / v.length : null;
  };
  const avgEffort = avg("effort"), avgMood = avg("mood");
  const avgEnergy = avg("energy"), avgSleep = avg("sleepQuality");
  const avgSoreness = avg("soreness");
  const radarData = [
    { subject: t.effort ?? "Effort",     value: +(avgEffort ?? 0).toFixed(1),                                fullMark: 5 },
    { subject: t.mood ?? "Mood",         value: +(avgMood ?? 0).toFixed(1),                                  fullMark: 5 },
    { subject: t.energy ?? "Energy",     value: +(avgEnergy ?? 0).toFixed(1),                                fullMark: 5 },
    { subject: t.sleep ?? "Sleep",       value: +(avgSleep ?? 0).toFixed(1),                                 fullMark: 5 },
    { subject: t.recovery ?? "Recovery", value: avgSoreness != null ? +(6 - avgSoreness).toFixed(1) : 5,    fullMark: 5 },
  ];

  // Category breakdown — use ALL_CATEGORIES order so slices appear
  // consistently across reports. Filter to non-zero before passing to
  // recharts (it doesn't render empty slices well).
  const catTotals = {};
  logs.forEach(l => {
  ...
  const categoryData = ALL_CATEGORIES
    .map((name) => ({ name, value: catTotals[name] || 0 }))
    .filter((x) => x.value > 0);
  ...
  const restqData = subs ? Object.entries(subs).map(([key, val]) => ({
    subject: key.replace(/([A-Z])/g, " $1").replace(/^./, c => c.toUpperCase()),
    value: typeof val === "number" ? +val.toFixed(2) : 0,
    fullMark: 6,
  })) : [];

  // Goals radar — 5 axes from latestGoals.answers, with question 4 inverted
  // (high obstacles = bad, so flip so bigger pentagon = healthier)
  ...
  const goalsData = goalsAnswers.length === 5
  ...
  return (
    <div style={wrap}>
      {/* Wellbeing trends */}
      <div id="pdf-chart-trends" style={{
        width: Math.max(520, trendsData.length * 22), height: 220,
        background: "#fff", padding: "8px",
      }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={trendsData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e0e8f0" vertical={false} />
            <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#7a9ab8" }} tickLine={false} axisLine={false}
              interval={Math.max(0, Math.ceil(trendsData.length / 12) - 1)} />
            <YAxis domain={[0, 5]} ticks={[1, 2, 3, 4, 5]}
              tick={{ fontSize: 10, fill: "#7a9ab8" }} tickLine={false} axisLine={false} />
            <Tooltip /><Legend wrapperStyle={{ fontSize: 11, paddingTop: 4 }} />
            <ReferenceLine y={3} stroke="#d0dcea" strokeDasharray="4 4" />
            <Line type="monotone" dataKey="effort" name={t.effort ?? "Effort"} stroke="#4a7ab5" strokeWidth={2} dot={{ r: 3, fill: "#4a7ab5", strokeWidth: 0 }} connectNulls />
            <Line type="monotone" dataKey="mood"   name={t.mood ?? "Mood"}     stroke="#10b981" strokeWidth={2} dot={{ r: 3, fill: "#10b981", strokeWidth: 0 }} connectNulls />
            <Line type="monotone" dataKey="energy" name={t.energy ?? "Energy"} stroke="#f59e0b" strokeWidth={2} dot={{ r: 3, fill: "#f59e0b", strokeWidth: 0 }} connectNulls />
            <Line type="monotone" dataKey="sleep"  name={t.sleep ?? "Sleep"}   stroke="#8b5cf6" strokeWidth={2} dot={{ r: 3, fill: "#8b5cf6", strokeWidth: 0 }} connectNulls />
          </LineChart>
        </ResponsiveContainer>
      </div>

      {/* Wellbeing radar */}
      <div id="pdf-chart-radar" style={{ width: 320, height: 280, background: "#fff", padding: "8px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#2d4a6e", textAlign: "center", marginBottom: 4 }}>
        ...
        <ResponsiveContainer width="100%" height="90%">
          <RadarChart data={radarData} margin={{ top: 16, right: 44, bottom: 16, left: 44 }}>
            ...
            <Radar name={t.average ?? "Average"} dataKey="value" stroke="#4a7ab5" fill="#4a7ab5" fillOpacity={0.3} strokeWidth={2} />
          </RadarChart>
        </ResponsiveContainer>
      </div>

      {/* Goals radar — for the period stats section */}
      {goalsData.length > 0 && (
        <div id="pdf-chart-goals" style={{ width: 320, height: 280, background: "#fff", padding: "8px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#2d4a6e", textAlign: "center", marginBottom: 4 }}>
          ...
          <ResponsiveContainer width="100%" height="90%">
            <RadarChart data={goalsData} margin={{ top: 16, right: 36, bottom: 16, left: 36 }}>
              <PolarGrid stroke="#d0dcea" />
              <PolarAngleAxis dataKey="subject" tick={{ fontSize: 10, fill: "#2d4a6e", fontWeight: 600 }} />
              <PolarRadiusAxis angle={90} domain={[0, 5]} tickCount={6} tick={{ fontSize: 8, fill: "#7a9ab8" }} />
              <Radar name={t.score ?? "Score"} dataKey="value" stroke={goalsColor} fill={goalsColor} fillOpacity={0.3} strokeWidth={2} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div id="pdf-chart-category" style={{ width: 320, height: 320, background: "#fff", padding: "8px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#2d4a6e", textAlign: "center", marginBottom: 4 }}>
        ...
        <div style={{ width: "100%", height: 200 }}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              >
                {(categoryData.length > 0 ? categoryData : [{ name: "empty" }]).map((entry, i) => (
                  <Cell
                    key={`${entry.name}-${i}`}
                    fill={
                      entry.name === "empty"
      ...
      {/* Weekly volume bars */}
      {volumeData.length > 0 && (
        <div id="pdf-chart-volume" style={{
          width: Math.max(520, volumeData.length * 40), height: 220,
          background: "#fff", padding: "8px",
        }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={volumeData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 10, fill: "#7a9ab8" }} tickLine={false} axisLine={false} />
              <Tooltip />
              <Bar dataKey="minutes" name={t.minutes ?? "Minutes"} fill="#4a7ab5" radius={[3, 3, 0, 0]} maxBarSize={32} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {/* Composite score timeline */}
      {scoresData.length > 0 && (
        <div id="pdf-chart-scores" style={{
          width: Math.max(520, scoresData.length * 22), height: 200,
          background: "#fff", padding: "8px",
        }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={scoresData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e0e8f0" vertical={false} />
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#7a9ab8" }} tickLine={false} axisLine={false}
                interval={Math.max(0, Math.ceil(scoresData.length / 12) - 1)} />
              <YAxis domain={[0, 100]} tick={{ fontSize: 10, fill: "#7a9ab8" }} tickLine={false} axisLine={false} />
              <Tooltip /><Legend wrapperStyle={{ fontSize: 11, paddingTop: 4 }} />
              <Line type="monotone" dataKey="composite" name={t.composite ?? "Composite"} stroke="#2d4a6e" strokeWidth={2.5} dot={{ r: 3, fill: "#2d4a6e", strokeWidth: 0 }} />
              <Line type="monotone" dataKey="wellbeing" name={t.wellbeing ?? "Wellbeing"} stroke="#10b981" strokeWidth={1.5} dot={false} />
              <Line type="monotone" dataKey="workout"   name={t.workout ?? "Workout"}     stroke="#4a7ab5" strokeWidth={1.5} dot={false} />
              <Line type="monotone" dataKey="sleep"     name={t.sleep ?? "Sleep"}         stroke="#8b5cf6" strokeWidth={1.5} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {/* RestQ subscales radar */}
      {restqData.length > 0 && (
        <div id="pdf-chart-restq" style={{ width: 480, height: 360, background: "#fff", padding: "8px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#2d4a6e", textAlign: "center", marginBottom: 4 }}>
            {t.restqSubscales ?? "RestQ Subscales"}
          </div>
          <ResponsiveContainer width="100%" height="90%">
            <RadarChart data={restqData} margin={{ top: 24, right: 60, bottom: 24, left: 60 }}>
              <PolarGrid stroke="#d0dcea" />
              <PolarAngleAxis dataKey="subject" tick={{ fontSize: 9, fill: "#2d4a6e", fontWeight: 600 }} />
              <PolarRadiusAxis angle={90} domain={[0, 6]} tickCount={4} tick={{ fontSize: 8, fill: "#7a9ab8" }} />
              <Radar name={t.score ?? "Score"} dataKey="value" stroke="#4a7ab5" fill="#4a7ab5" fillOpacity={0.25} strokeWidth={2} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
      )}
...
export default function PdfExportModal({ data, t: tProp, onClose }) {
  ...
  const handleGenerate = useCallback(async () => {
    setLoading(true); setError(""); setStep(t.renderingCharts ?? "Rendering charts…");
    setShowCharts(true);
    try {
      await new Promise(r => setTimeout(r, 900));
      setStep(t.capturingDiagrams ?? "Capturing diagrams…");
      await generatePDF({ data, t, year, month, allTime, logs });
      setShowCharts(false);
      onClose();
    } catch (e) {
      console.error(e);
  ...
  }, [data, t, year, month, allTime, logs, onClose]);
    ...
    <>
      ...
      <div onClick={onClose} style={{
        ...
        <div onClick={e => e.stopPropagation()} style={{
          background: SU, borderRadius: 18, width: "100%", maxWidth: 400,
          boxShadow: "0 24px 60px rgba(45,74,110,0.25)",
          border: `1px solid ${BO}`, overflow: "hidden",
        }}>
          ...
          <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
            <label style={{
              ...
              fontSize: 13, color: AD, fontWeight: 600,
            }}>
            ...
            {!allTime && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: MU, letterSpacing: 0.8, marginBottom: 5 }}>
                    {(t.month ?? "MONTH").toUpperCase()}
                  </div>
            ...
            <div style={{
              ...
              ✓ {t.client ?? "Client"} · ✓ {t.bodyMetrics ?? "Body metrics"}<br />
              ✓ 🎯 {t.goalsTitle ?? "Goal check-in"} radar<br />
              ✓ 📈 {t.wellbeingTrends ?? "Wellbeing trends"} ({t.effort ?? "Effort"}, {t.mood ?? "Mood"}, {t.energy ?? "Energy"}, {t.sleep ?? "Sleep"})<br />
              ✓ 💡 {t.relevantAdvice ?? "Relevant tips"}<br />
              ✓ 🕸 {t.wellbeingBalance ?? "Wellbeing radar"}<br />
              ✓ 🥧 {t.categoryMix ?? "Category mix"} donut<br />
              ✓ 📊 {t.weeklyVolume ?? "Weekly volume"} bars<br />
              ✓ 📈 {t.compositeScore ?? "Composite score"} timeline<br />
              ✓ 🕸 {t.restqSubscales ?? "RestQ subscales"} radar<br />
              ✓ {t.recoveryState ?? "Recovery state"} (Hooper + RestQ)<br />
              ✓ {t.history ?? "Full log"}
            </div>
            ...
            <button onClick={handleGenerate} disabled={loading} style={{
              background: `linear-gradient(135deg,${A},${AD})`, color: "#fff",
              border: "none", borderRadius: 10, padding: "13px", fontSize: 13,
              fontWeight: 700, cursor: loading ? "wait" : "pointer",
              fontFamily: "inherit", opacity: loading ? 0.7 : 1,
              transition: "opacity .15s",
            }}>
              {loading ? `⏳ ${step || (t.generating ?? "Generating…")}` : `⬇ ${t.downloadPdf ?? "Download PDF"}`}
            </button>
          </div>
        </div>
      </div>
