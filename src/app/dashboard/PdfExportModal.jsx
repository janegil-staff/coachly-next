...
async function generatePDF({ data, t, year, month, allTime, logs }) {
  ...
  const [trendsImg, radarImg, categoryImg, volumeImg, scoresImg, restqImg] = await Promise.all([
  ...
  const wRows = [
    ...
    [t.proStatus ?? "Pro",     profile.isPro ? (t.yes ?? "Yes") : (t.no ?? "No")],
  ];
  wRows.forEach(([l, v], i) => {
  ...
  row(t.daysLogged ?? "Days logged",   logs.length, false);
  row(t.sessionsLogged ?? "Sessions",  sessionsCount, true);
  row(t.totalHours ?? "Total hours",   (totalMin / 60).toFixed(1), false);
  row(t.restDays ?? "Rest days",       restCount, true);
  row(t.avgEffort ?? "Avg effort",     avgOf(logs, "effort"), false);
  row(t.avgMood ?? "Avg mood",         avgOf(logs, "mood"), true);
  row(t.avgEnergy ?? "Avg energy",     avgOf(logs, "energy"), false);
  row(t.avgSleep ?? "Avg sleep",       avgOf(logs, "sleepQuality"), true);
  row(t.avgSoreness ?? "Avg soreness", avgOf(logs, "soreness"), false);
  row(t.totalRecords ?? "Total records (all time)", allLogs.length, true);
  y += 3;
  ...
  if (notesLogs.length > 0) {
    ...
    notesLogs.forEach((l, i) => {
      checkPage(8);
      if (i % 2 === 0) { doc.setFillColor(...LGRAY); doc.rect(ML, y, CW, 7, "F"); }
  ...
  return (
    <div style={wrap}>
      ...
      {categoryData.length > 0 && (
        <div id="pdf-chart-category" style={{ width: 320, height: 280, background: "#fff", padding: "8px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#2d4a6e", textAlign: "center", marginBottom: 4 }}>
            ...
            <PieChart>
              <Pie data={categoryData} dataKey="value" nameKey="name" cx="50%" cy="50%"
                ...
                {categoryData.map((entry, i) => (
                  <Cell key={i} fill={CAT_COLORS[entry.name] ?? CAT_COLORS.other} />
                ))}
              </Pie>
              <Tooltip />
            </PieChart>
        ...
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{
              ...
              ✓ {t.recentNotes ?? "Recent notes"} · ✓ {t.history ?? "Full log"}
            </div>
