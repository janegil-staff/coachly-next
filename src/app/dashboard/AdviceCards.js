...
//
// Renders advice items the user has marked as "relevant for me" in the
// mobile app. Reads from report.client.profile.relevantAdvice (an array
// of advice IDs like ["t1", "r1", "r2"]) and looks each one up.
//
...
export default function AdviceCards({ relevantAdvice, t }) {
  ...
  // Resolve IDs → items. Translation values (if present) override fallbacks.
    ...
    return (
      ...
      >
        ...
        <div className="text-xs" style={{ color: MU }}>
          {t.adviceEmpty ?? "No tips marked as relevant."}
        </div>
      </div>
    );
  }
